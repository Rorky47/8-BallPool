...
import { PROTOCOL_VERSION, isClientToServer } from "@eightball/shared";
...
async function main() {
    ...
    const rooms = new Map();
    const playerToRoom = new Map(); // token -> roomId
    const emit = (socketId, msg) => {
        io.to(socketId).emit(SOCKET_EVENT, msg);
    };
    io.on("connection", (socket) => {
        const auth = socket.handshake.auth;
        const token = typeof auth === "object" &&
            auth !== null &&
            "token" in auth &&
        ...
        socket.on(SOCKET_EVENT, (raw) => {
            ...
            if (raw.t === "queue/leave") {
                ...
                socket.emit(SOCKET_EVENT, { t: "queue/status", inQueue: false });
                return;
            }
            if (raw.t === "queue/join") {
                ...
                socket.emit(SOCKET_EVENT, { t: "queue/status", inQueue: true, position: pos });
                ...
                if (match) {
                    ...
                    room.start();
                    emit(a.socket.id, { t: "match/found", roomId, playerIndex: 0 });
                    emit(b.socket.id, { t: "match/found", roomId, playerIndex: 1 });
