import { DEFAULT_PHYSICS_PARAMS, analyzeShot, createInitialEightBallRulesState, stepBallsInPlace } from "@eightball/shared";
...
export class GameRoom {
    ...
    ended = false;
    state;
    rules = createInitialEightBallRulesState();
    shot = null;
    interval = null;
    dt = 1 / 120;
    tickEveryMs = 8;
    reconnectWindowMs = 20_000;
    constructor(io, id, p0Token, p1Token, onEnd = () => { }) {
        this.io = io;
        this.id = id;
        this.onEnd = onEnd;
        this.players = [
    ...
    handleDisconnect(token) {
        ...
        if (idx === null)
            return;
        const slot = this.players[idx];
        if (slot.socket?.id === null)
            return;
        slot.socket = null;
        slot.disconnectedAtMs = Date.now();
    }
    handleCommand(token, msg) {
        const idx = this.playerIndexOf(token);
        if (idx === null)
            return;
        if (msg.t === "game/aim") {
            if (this.state.phase !== "aim" && this.state.phase !== "ball_in_hand")
                return;
            if (idx !== this.state.currentPlayer)
                return;
        ...
        if (msg.t === "game/place_cue") {
            if (this.state.phase !== "ball_in_hand")
                return;
            if (idx !== this.state.currentPlayer)
                return;
    ...
    tick() {
        ...
        if (this.state.phase === "simulating") {
            ...
            const res = stepBallsInPlace(this.state.balls, this.state.table, this.dt, DEFAULT_PHYSICS_PARAMS);
            if (shot)
                shot.events.push(...res.events);
            this.state.tick++;
            if (!res.anyMoving) {
